package com.dougmccune.containers
{
	...
	public class BasePV3DContainer extends ViewStack
	{
		...
		/**
		* Timer to change the selected index in case of auto select.
		*
		*/
		private var rTimer:Timer;
		
		private var autoRotateChanged:Boolean = false;
		...
		private var _delay:Number = 4000;
		...
		private function showVisibleChild():void {
			if(selectedChild != null) {
				...
				var plane:DisplayObject3D = lookupPlane(selectedChild);
		...
		override protected function commitProperties():void
		{
			...
			if(autoRotateChanged)
			{
				if(autoRotate)
				{
					if(!rTimer)
					{
						rTimer = new Timer(delay);
						rTimer.addEventListener(TimerEvent.TIMER, onRotateTimer_Handler);
		...
		public function restartAutoRotateTimer():void
		{
			if(!rTimer) return;
		...
		private function onRotateTimer_Handler(event:TimerEvent):void
		{
			
			if(this.selectedIndex <= this.numChildren-2)
		...
		{
			trace("Rotating Timer Complete"+event.toString());
			if(rTimer)
			{
				dispose();
			}
		...
		{
			if(autoRotate && rTimer && rTimer.running)
				rTimer.reset();
		}
		...
		private function dispose():void
		{
			...
			{
				rTimer.stop();
				rTimer.removeEventListener(TimerEvent.TIMER, onRotateTimer_Handler);
				rTimer.removeEventListener(TimerEvent.TIMER_COMPLETE, onTimer_CompleteHandler);
				rTimer = null;
			}
		}
	}
